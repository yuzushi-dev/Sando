...
const TOOL = {
  ...
  inputSchema: { type: 'object', additionalProperties: false, required: ['toolName', 'output', 'cwd'], properties: { toolName: { type: 'string' }, output: {}, cwd: { type: 'string' }, policy: { type: 'object' } } },
...
function dispatch(message) {
  ...
  if (message.method === 'tools/call') {
    ...
    try { const result = optimizeToolOutput(message.params.arguments); return response(message.id, { content: [{ type: 'text', text: result.inline }], structuredContent: result, isError: false }); }
    catch (cause) { return response(message.id, { content: [{ type: 'text', text: cause instanceof Error ? cause.message : 'invalid tool input' }], isError: true }); }
  }
  return error(message.id, -32601, 'Method not found');
}
export function startMcpServer() {
  const lines = readline.createInterface({ input: process.stdin, crlfDelay: Infinity });
  lines.on('line', (line) => { let output; try { output = dispatch(JSON.parse(line)); } catch { output = error(null, -32700, 'Parse error'); } if (output) process.stdout.write(`${JSON.stringify(output)}\\n`); });
}
